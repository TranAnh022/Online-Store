...
const Rating: React.FC<RatingProps> = ({
  ...
  const getColor = (index: number) => {
    if (hoverValue !== undefined && index <= hoverValue) return hoverColor;
    if (value > index) return activeColor;
    return color;
  };
  ...
  for (let i = 0; i < count; i++) {
    ...
    stars.push(
      ...
      >
        {React.cloneElement(star, { size, color: getColor(i) })}
      </div>
    );
  }

  return (
    <div className={`rating ${className}`} style={{ display: "flex" }}>
      {stars}
    </div>
  );
};
